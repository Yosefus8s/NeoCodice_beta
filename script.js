        ...
        interactables: [
            {
                ...
                action: () => { // Função a ser executada na interação
                    console.log("Interagiu com o Livro do Egito!");
                    mostrarCenaComDialogo(
                        'https://uploads.onecompiler.io/43rzumf93/44293xhug/livrobt.png', // imagem que quiser
                        'Você abriu o Livro do Egito... prepare-se para a jornada!',
                        () => loadLevel('egypt')
                    );
                }
            }
...
const player = {
    ...
    speed: 10, state: 'idleDown', direction: 'down',
...
window.addEventListener('keydown', (event) => {
    ...
    // Lógica existente para movimento e interação DURANTE o jogo
    if (event.key in keys) {
        event.preventDefault();
        keys[event.key] = true;
    }
    // Modificado: só chama handleInteraction se o livro NÃO estiver a ser mostrado
...
function handleInteraction() {
    ...
    console.log("Nenhum item interativo.");
}
...
function mostrarCenaComDialogo(imagemUrl, textoDialogo, callback = null) {
    // Cria container principal (overlay)
    ...
    setTimeout(() => { imagem.style.opacity = '1'; }, 50);

    // Cria faixa de diálogo (em azul translúcido)
    const faixa = document.createElement('div');
    faixa.style.position = 'absolute';
    faixa.style.bottom = '0';
    faixa.style.left = '50%';
    faixa.style.transform = 'translateX(-50%)';
    faixa.style.width = '90vw';
    faixa.style.padding = '25px';
    faixa.style.background = 'rgba(0, 60, 160, 0.3)';
    faixa.style.color = 'white';
    faixa.style.border = '2px solid gray';
 faixa.style.fontFamily = 'Press Start 2P, cursive';
    ...
    faixa.style.animation = 'slideUp 0.5s ease';
    ...
    // Função de digitação gradual do texto
    let i = 0;
    let typeTimer = null;
    function escreverTexto() {
        if (i < textoDialogo.length) {
            ...
            i++;
            typeTimer = setTimeout(escreverTexto, 40); // velocidade do efeito
        }
    }
    escreverTexto();
    ...
    const fecharCena = (e) => {
        // 4. Impede que o evento "borbulhe" e ative outros listeners
        e.stopPropagation();
        e.preventDefault();

        // 5. Para o timer da digitação (evita erros no console)
        if (typeTimer) {
            clearTimeout(typeTimer);
        }

        // 6. Remove os listeners IMEDIATAMENTE (não dentro do timeout)
        container.removeEventListener('click', fecharCena);
        document.removeEventListener('keydown', fecharCena);

        // Animação de saída
        container.style.animation = 'fadeOut 0.5s ease';
        setTimeout(() => {
            container.remove();
            // O removeEventListener que estava aqui foi movido para cima
            if (typeof callback === 'function') callback();
        }, 400);
    };

    container.addEventListener('click', fecharCena);
    document.addEventListener('keydown', fecharCena);
}
